...
Visualization: heatmap for drought scales summed by state from `drought_pop_total`

```{r}
drought_categories <- c("D0", "D1", "D2", "D3", "D4")

state_summary_pt <- drought_pop_total %>%
group_by(state_abb) %>% summarise(across(all_of(drought_categories), sum, na.rm = TRUE))

state_summary_pt[1:10,]
```
```{r}
state_summary_long_pt <- state_summary_pt %>%
pivot_longer(cols = all_of(drought_categories), names_to = "Category", values_to = "Pop")

ggplot(state_summary_long_pt, aes(x = Category, y = state_abb, fill = Pop)) + 
  geom_tile() +
...
Visualization: reported state population affected by each drought category over time
```{r}
...
ggplot(drought_pop_total, aes(x=map_date)) + 
  ...
  labs(title="Drought Categories Effect on Population Over Time",
       x="Date",
       y = "Area") +
scale_color_manual(values = c("red", "blue", "green", "purple", "orange")) + theme_minimal()
```


From `drought_area_total` let's observe a State-level summary of how much total land area has been affected by each drought category

```{r}
drought_categories <- c("D0", "D1", "D2", "D3", "D4")
state_summary_at <- drought_area_total %>%
  group_by(state_abb) %>% 
  summarise(across(all_of(drought_categories), sum, na.rm = TRUE))
state_summary_at[1:10,]
```



Visualization: heatmap for total land area by state affected by each drought category, from `drought_area_total`
```{r}
state_summary_long_at <- state_summary_at %>%
pivot_longer(cols = all_of(drought_categories), names_to = "Category", values_to = "Area")

ggplot(state_summary_long_at, aes(x = Category, y = state_abb, fill = Area)) + 
  geom_tile() +
  scale_fill_gradient(low = "white", high = "red") +
  labs(title = "Drought Categories by State (Sum)",
       x = "Drought Category",
